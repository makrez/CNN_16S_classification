...
def main(labels_file, taxonomic_level, taxonomic_group, 
         ...
         fraction, output_path, make_plot=False):
    """
    This function summarises the number of sequences per class at a certain 
    # classification level within a taxonomic group. 
    # Example: {taxonomic_level Domain, taxonomic_group Bacteria, 
    # classification_level Phylum} will output the number of sequences per phylum
    # within Bacteria.

    # Args:
    #   labels_file: path to .pkl file with the labels
    #   taxonomic level: The taxonomic level of the outer group (e.g. Domain)
    #   txonomic_group: Name of the group in taxononic_level (e.g. Bacteria)
    #   classification_level: Inner group. To which classification level do the
    #       classes belong.
    #   minimum_samples: Minimum samples per class to be considered
    #   fraction: fraction of total sequences to be included
    #   output_path: Path to where the outputs are saved
    #   make_plot: True/False if a plot should be made
    #
    # Output:
    #   csv-file containing the class counts
    #   png-file with histogram if make_plot=True
    """
